	...
	public class FastZip
	{
		...
		public void CreateZip(Stream outputStream, string sourceDirectory, bool recurse, string fileFilter, string directoryFilter)
		{
			...
			using (this.outputStream_ = new ZipOutputStream(outputStream))
			{
				...
				FileSystemScanner expr_64 = fileSystemScanner;
				expr_64.ProcessFile = (ProcessFileHandler)Delegate.Combine(expr_64.ProcessFile, new ProcessFileHandler(this.ProcessFile));
				if (this.CreateEmptyDirectories)
				{
					FileSystemScanner expr_94 = fileSystemScanner;
					expr_94.ProcessDirectory = (ProcessDirectoryHandler)Delegate.Combine(expr_94.ProcessDirectory, new ProcessDirectoryHandler(this.ProcessDirectory));
				}
				if (this.events_ != null)
				{
					if (this.events_.FileFailure != null)
					{
						FileSystemScanner expr_D8 = fileSystemScanner;
						expr_D8.FileFailure = (FileFailureHandler)Delegate.Combine(expr_D8.FileFailure, this.events_.FileFailure);
					}
					if (this.events_.DirectoryFailure != null)
					{
						FileSystemScanner expr_10D = fileSystemScanner;
						expr_10D.DirectoryFailure = (DirectoryFailureHandler)Delegate.Combine(expr_10D.DirectoryFailure, this.events_.DirectoryFailure);
					}
				}
		...
		{
			bool flag = true;
			if (this.overwrite_ != FastZip.Overwrite.Always)
			{
