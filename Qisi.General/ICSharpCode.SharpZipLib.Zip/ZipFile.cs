...
namespace ICSharpCode.SharpZipLib.Zip
{
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		private class PartialInputStream : Stream
		{
			...
			public override int ReadByte()
			{
				...
				else
				{
					...
					try
					{
						Stream arg_40_0 = this.baseStream_;
						long offset;
						this.readPos_ = (offset = this.readPos_) + 1L;
						arg_40_0.Seek(offset, SeekOrigin.Begin);
		...
		private void RunUpdates()
		{
			...
			try
			{
				...
				foreach (ZipFile.ZipUpdate zipUpdate in this.updates_)
				{
					if (zipUpdate != null)
					{
						num += (long)zipFile.WriteCentralDirectoryHeader(zipUpdate.OutEntry);
					}
				}
				byte[] comment = (this.newComment_ != null) ? this.newComment_.RawComment : ZipConstants.ConvertToArray(this.comment_);
				...
				foreach (ZipFile.ZipUpdate zipUpdate in this.updates_)
				{
					if (zipUpdate != null)
					{
						...
						{
							zipFile.baseStream_.Position = zipUpdate.CrcPatchOffset;
							zipFile.WriteLEInt((int)zipUpdate.OutEntry.Crc);
						}
						if (zipUpdate.SizePatchOffset > 0L)
						{
							zipFile.baseStream_.Position = zipUpdate.SizePatchOffset;
							if (zipUpdate.OutEntry.LocalHeaderRequiresZip64)
							{
								zipFile.WriteLeLong(zipUpdate.OutEntry.Size);
								zipFile.WriteLeLong(zipUpdate.OutEntry.CompressedSize);
							}
							else
							{
								zipFile.WriteLEInt((int)zipUpdate.OutEntry.CompressedSize);
								zipFile.WriteLEInt((int)zipUpdate.OutEntry.Size);
							}
						}
					}
				}
		...
		private void ReadEntries()
		{
			...
			for (ulong num16 = 0uL; num16 < num4; num16 += 1uL)
			{
				...
				uint num19 = this.ReadLEUint();
				long num20 = (long)((ulong)this.ReadLEUint());
				long num21 = (long)((ulong)this.ReadLEUint());
				...
				byte[] array2 = new byte[Math.Max(num22, num24)];
				StreamUtils.ReadFully(this.baseStream_, array2, 0, num22);
				string name = ZipConstants.ConvertToStringExt(num17, array2, num22);
				ZipEntry zipEntry = new ZipEntry(name, versionRequiredToExtract, madeByInfo, (CompressionMethod)method);
				zipEntry.Crc = (long)((ulong)num19 & (ulong)-1);
				zipEntry.Size = (num21 & (long)((ulong)-1));
				zipEntry.CompressedSize = (num20 & (long)((ulong)-1));
				zipEntry.Flags = num17;
				zipEntry.DosTime = (long)((ulong)num18);
				zipEntry.ZipFileIndex = (long)num16;
				zipEntry.Offset = num9;
				zipEntry.ExternalFileAttributes = (int)externalFileAttributes;
				if ((num17 & 8) == 0)
				{
					zipEntry.CryptoCheckValue = (byte)(num19 >> 24);
				}
				else
				{
					zipEntry.CryptoCheckValue = (byte)(num18 >> 8 & 255u);
				}
				if (num23 > 0)
				{
					byte[] array3 = new byte[num23];
				...
				if (num24 > 0)
				{
					...
					zipEntry.Comment = ZipConstants.ConvertToStringExt(num17, array2, num24);
				}
				this.entries_[(int)checked((IntPtr)num16)] = zipEntry;
			}
		}
