...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
	public class DeflaterEngine : DeflaterConstants
	{
		...
		private void SlideWindow()
		{
			...
			for (int i = 0; i < 32768; i++)
			{
				int num = (int)this.prev[i] & 65535;
				this.prev[i] = (short)((num >= 32768) ? (num - 32768) : 0);
			}
		}
			...
			else
			{
				while (this.lookahead >= 262 || flush)
				{
					...
					if (this.lookahead >= 3)
					{
						...
						if (this.strategy != DeflateStrategy.HuffmanOnly && num3 != 0 && this.strstart - num3 <= 32506 && this.FindLongestMatch(num3))
						{
							if (this.matchLen <= 5 && (this.strategy == DeflateStrategy.Filtered || (this.matchLen == 3 && this.strstart - this.matchStart > 4096)))
							{
								this.matchLen = 2;
							}
						}
